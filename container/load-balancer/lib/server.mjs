...
app.get("*", async (req, res) => {
    ...
    const index = isNaN(affinity) ? Math.floor(Math.random() * serverInstances.length) : affinity;
    ...
    console.log(`Proxy request ${req.path} ->`, url.toString(), `| Affintiy: ${affinity}`, ` | Actual: ${index}`);
